...
GlyphRenderer::GlyphRenderer(Renderer *renderer)
    : ref_count(0)
{
...
HRESULT GlyphRenderer::DrawGlyphRun(void *client_drawing_context, float baseline_origin_x, float baseline_origin_y, DWRITE_MEASURING_MODE measuring_mode,
    DWRITE_GLYPH_RUN const *glyph_run, DWRITE_GLYPH_RUN_DESCRIPTION const *glyph_run_description, IUnknown *client_drawing_effect) noexcept
{
    ...
    DWRITE_GLYPH_IMAGE_FORMATS supported_formats = DWRITE_GLYPH_IMAGE_FORMATS_TRUETYPE | DWRITE_GLYPH_IMAGE_FORMATS_CFF | DWRITE_GLYPH_IMAGE_FORMATS_COLR |
                                                   DWRITE_GLYPH_IMAGE_FORMATS_SVG | DWRITE_GLYPH_IMAGE_FORMATS_PNG | DWRITE_GLYPH_IMAGE_FORMATS_JPEG |
    ...
    hr = renderer->dwrite_factory->TranslateColorGlyphRun(D2D1_POINT_2F{ .x = baseline_origin_x, .y = baseline_origin_y }, glyph_run, glyph_run_description,
        supported_formats, measuring_mode, nullptr, 0, &glyph_run_enumerator);

    if (hr == DWRITE_E_NOCOLOR)
    {
        renderer->d2d_context->DrawGlyphRun(D2D1_POINT_2F{ .x = baseline_origin_x, .y = baseline_origin_y }, glyph_run, drawing_effect_brush, measuring_mode);
    }
    else
    {
        assert(!FAILED(hr));
        ...
        while (true)
        {
            ...
            D2D1_POINT_2F current_baseline_origin{ .x = color_run->baselineOriginX, .y = color_run->baselineOriginY };
            ...
            case DWRITE_GLYPH_IMAGE_FORMATS_PREMULTIPLIED_B8G8R8A8: {
                renderer->d2d_context->DrawColorBitmapGlyphRun(color_run->glyphImageFormat, current_baseline_origin, &color_run->glyphRun, measuring_mode);
            }
            break;
            case DWRITE_GLYPH_IMAGE_FORMATS_SVG: {
                renderer->d2d_context->DrawSvgGlyphRun(current_baseline_origin, &color_run->glyphRun, drawing_effect_brush, nullptr, 0, measuring_mode);
            }
            break;
            ...
            default: {
                ...
                renderer->d2d_context->DrawGlyphRun(current_baseline_origin, &color_run->glyphRun, color_run->glyphRunDescription,
                    use_palette_color ? temp_brush : drawing_effect_brush, measuring_mode);
...
HRESULT GlyphRenderer::DrawInlineObject(void *client_drawing_context, float origin_x, float origin_y, IDWriteInlineObject *inline_obj, BOOL is_sideways,
    BOOL is_right_to_left, IUnknown *client_drawing_effect) noexcept
{
    return E_NOTIMPL;
}

HRESULT GlyphRenderer::DrawLine(void *client_drawing_context, float baseline_origin_x, float baseline_origin_y, FLOAT offset, FLOAT width, FLOAT thickness,
    IUnknown *client_drawing_effect, bool use_special_color) noexcept
{
    ...
    D2D1_RECT_F rect = D2D1_RECT_F{ .left = baseline_origin_x,
...
HRESULT GlyphRenderer::DrawStrikethrough(void *client_drawing_context, float baseline_origin_x, float baseline_origin_y,
    DWRITE_STRIKETHROUGH const *strikethrough, IUnknown *client_drawing_effect) noexcept
{

    return DrawLine(client_drawing_context, baseline_origin_x, baseline_origin_y, strikethrough->offset, strikethrough->width, strikethrough->thickness,
        client_drawing_effect, false);
}

HRESULT GlyphRenderer::DrawUnderline(
    void *client_drawing_context, float baseline_origin_x, float baseline_origin_y, DWRITE_UNDERLINE const *underline, IUnknown *client_drawing_effect) noexcept
{

    return DrawLine(
        client_drawing_context, baseline_origin_x, baseline_origin_y, underline->offset, underline->width, underline->thickness, client_drawing_effect, true);
}
