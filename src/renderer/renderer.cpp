...
void InitializeD3D(Renderer *renderer)
{
    ...
    WIN_CHECK(D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, flags, feature_levels, ARRAYSIZE(feature_levels), D3D11_SDK_VERSION, &temp_device,
...
void InitializeDWrite(Renderer *renderer)
{
    ...
    if (renderer->disable_ligatures)
    {
        ...
        WIN_CHECK(renderer->dwrite_typography->AddFontFeature(DWRITE_FONT_FEATURE{ .nameTag = DWRITE_FONT_FEATURE_TAG_STANDARD_LIGATURES, .parameter = 0 }));
    }
}
...
void InitializeWindowDependentResources(Renderer *renderer, uint32_t width, uint32_t height)
{
    ...
    if (renderer->dxgi_swapchain)
    {
        ...
        HRESULT hr = renderer->dxgi_swapchain->ResizeBuffers(
            2, width, height, DXGI_FORMAT_B8G8R8A8_UNORM, DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT | DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING);
    ...
    {
        DXGI_SWAP_CHAIN_DESC1 swapchain_desc{ .Width = width,
            .Height = height,
        ...
        WIN_CHECK(dxgi_factory->CreateSwapChainForHwnd(renderer->d3d_device, renderer->hwnd, &swapchain_desc, nullptr, nullptr, &dxgi_swapchain_temp));
        WIN_CHECK(dxgi_factory->MakeWindowAssociation(renderer->hwnd, DXGI_MWA_NO_ALT_ENTER));
        WIN_CHECK(dxgi_swapchain_temp->QueryInterface(__uuidof(IDXGISwapChain2), reinterpret_cast<void **>(&renderer->dxgi_swapchain)));
    ...
    constexpr D2D1_BITMAP_PROPERTIES1 target_bitmap_properties{ .pixelFormat = D2D1_PIXEL_FORMAT{ .format = DXGI_FORMAT_B8G8R8A8_UNORM,
    ...
    WIN_CHECK(renderer->d2d_context->CreateBitmapFromDxgiSurface(dxgi_backbuffer, &target_bitmap_properties, &renderer->d2d_target_bitmap));
...
void HandleDeviceLost(Renderer *renderer)
{
    ...
    InitializeWindowDependentResources(
        renderer, static_cast<uint32_t>(client_rect.right - client_rect.left), static_cast<uint32_t>(client_rect.bottom - client_rect.top));
}
...
float GetTextWidth(Renderer *renderer, uint32_t *text, uint32_t length)
{
    ...
    WIN_CHECK(renderer->dwrite_factory->CreateTextLayout(
        renderer->wchar_buffer, renderer->wchar_buffer_length, renderer->dwrite_text_format, 0.0f, 0.0f, &test_text_layout));
...
bool UpdateFontMetrics(Renderer *renderer, float font_size, const char *font_string, int strlen)
{
    ...
    WIN_CHECK(renderer->dwrite_factory->CreateTextFormat(renderer->font, nullptr, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,
        DWRITE_FONT_STRETCH_NORMAL, renderer->font_size, L"en-us", &renderer->dwrite_text_format));

    WIN_CHECK(renderer->dwrite_text_format->SetLineSpacing(
        DWRITE_LINE_SPACING_METHOD_UNIFORM, renderer->font_height, renderer->font_ascent * renderer->linespace_factor));
...
void ApplyHighlightAttributes(Renderer *renderer, HighlightAttributes *hl_attribs, IDWriteTextLayout *text_layout, int start, int end)
{
    GlyphDrawingEffect *drawing_effect = new GlyphDrawingEffect(CreateForegroundColor(renderer, hl_attribs), CreateSpecialColor(renderer, hl_attribs));
    DWRITE_TEXT_RANGE range{ .startPosition = static_cast<uint32_t>(start), .length = static_cast<uint32_t>(end - start) };
    ...
    if (hl_attribs->flags & HL_ATTRIB_BOLD)
    {
        ...
        if (renderer->font_size != renderer->font_size_scale_bold) text_layout->SetFontSize(renderer->font_size_scale_bold, range);
    }
...
void DrawHighlightedText(Renderer *renderer, D2D1_RECT_F rect, uint32_t *text, uint32_t length, HighlightAttributes *hl_attribs)
{
    ...
    WIN_CHECK(renderer->dwrite_factory->CreateTextLayout(
        renderer->wchar_buffer, renderer->wchar_buffer_length, renderer->dwrite_text_format, rect.right - rect.left, rect.bottom - rect.top, &text_layout));
...
void DrawGridLine(Renderer *renderer, int row)
{
    ...
    WIN_CHECK(renderer->dwrite_factory->CreateTextLayout(renderer->wchar_buffer, renderer->wchar_buffer_length, renderer->dwrite_text_format,
    ...
    for (int i = 0, i_wchars = 0; i < renderer->grid_cols; i_wchars += ContainsSurrogatePair(renderer->grid_chars[base + i]) ? 2 : 1, ++i)
    {
    ...
    if (renderer->disable_ligatures)
    {
        text_layout->SetTypography(renderer->dwrite_typography, DWRITE_TEXT_RANGE{ .startPosition = 0, .length = static_cast<uint32_t>(grid_chars_length) });
    }
...
void DrawCursor(Renderer *renderer)
{
    ...
    if (cursor_grid_offset < (renderer->grid_rows * renderer->grid_cols) && renderer->grid_cell_properties[cursor_grid_offset].is_wide_char)
    {
    ...
    D2D1_RECT_F cursor_rect{ .left = renderer->cursor.col * renderer->font_width,
    ...
    {
        DrawHighlightedText(renderer, cursor_fg_rect, &renderer->grid_chars[cursor_grid_offset], double_width_char_factor, &cursor_hl_attribs);
    }
}
...
void UpdateGridSize(Renderer *renderer, mpack_node_t grid_resize)
{
    ...
    if (renderer->grid_chars == nullptr || renderer->wchar_buffer == nullptr || renderer->grid_cell_properties == nullptr || renderer->grid_cols != grid_cols ||
...
void UpdateImePos(Renderer *renderer)
{
    ...
    COMPOSITIONFORM composition_form{ .dwStyle = CFS_POINT,
        .ptCurrentPos = {
            .x = static_cast<LONG>(renderer->cursor.col * renderer->font_width), .y = static_cast<LONG>(renderer->cursor.row * renderer->font_height) } };

    if (ImmSetCompositionWindow(input_context, &composition_form))
    {
        LOGFONTW font_attribs{ .lfHeight = static_cast<LONG>(renderer->font_height) };
...
void ScrollRegion(Renderer *renderer, mpack_node_t scroll_region)
{
    ...
    for (size_t i = 1; i < scroll_count; ++i)
    {
        ...
        for (int64_t j = start_row; scrolling_down ? j <= end_row : j >= end_row; j += increment)
        {
            ...
            memcpy(&renderer->grid_chars[target_row * renderer->grid_cols + left], &renderer->grid_chars[j * renderer->grid_cols + left],
                (right - left) * sizeof(uint32_t));

            memcpy(&renderer->grid_cell_properties[target_row * renderer->grid_cols + left], &renderer->grid_cell_properties[j * renderer->grid_cols + left],
...
void DrawBorderRectangles(Renderer *renderer)
{
    ...
    if (left_border != static_cast<float>(renderer->pixel_size.width))
    {
        D2D1_RECT_F vertical_rect{
            .left = left_border, .top = 0.0f, .right = static_cast<float>(renderer->pixel_size.width), .bottom = static_cast<float>(renderer->pixel_size.height)
        };
    ...
    if (top_border != static_cast<float>(renderer->pixel_size.height))
    {
        D2D1_RECT_F horizontal_rect{
            .left = 0.0f, .top = top_border, .right = static_cast<float>(renderer->pixel_size.width), .bottom = static_cast<float>(renderer->pixel_size.height)
        };
...
void ClearGrid(Renderer *renderer)
{
    ...
    D2D1_RECT_F rect{ .left = 0.0f, .top = 0.0f, .right = renderer->grid_cols * renderer->font_width, .bottom = renderer->grid_rows * renderer->font_height };
...
PixelSize RendererGridToPixelSize(Renderer *renderer, int rows, int cols)
{
    ...
    return PixelSize{ .width = adjusted_rect.right - adjusted_rect.left, .height = adjusted_rect.bottom - adjusted_rect.top };
}

GridSize RendererPixelsToGridSize(Renderer *renderer, int width, int height)
{
    return GridSize{ .rows = static_cast<int>(height / renderer->font_height), .cols = static_cast<int>(width / renderer->font_width) };
}

GridPoint RendererCursorToGridPoint(Renderer *renderer, int x, int y)
{
    return GridPoint{ .row = static_cast<int>(y / renderer->font_height), .col = static_cast<int>(x / renderer->font_width) };
}
